...
int LoadMovieStateError;
static int FCEUMOV_CheckWrongMovie(FILE* st, uint32 size);
int WrongMovie;
int ShowMovieAVIMessage;
int MovieLoadedTotalFrames;
...
void MovieFlushHeader(void)
{
  ...
  moviedatasize = frameptr; // hack to prevent movie from looping endlessly after read-only toggle and then loading
...
//DEBUG_COMPARE_RAM(__LINE__);
  ...
  MovieLoadedTotalFrames = framecount;
  FCEUD_ResetStopframe(1);
  if (_stopframe) stopframe = _stopframe;
 ...
 if(current < 0)    // Playback
 {
  while(nextts == framets || nextd == -1)
  {
   ...
   nextd = d;
	...
	if (framets > nextts) // hack to prevent movie from looping when loading a savestate from another movie (mz)
		framets = nextts-1;
...
void FCEU_DrawMovies(uint8 *XBuf)
{
	...
	//|| input_display && (!movcounter || last_input_display!=cur_input_display)))
	{
		...
		*/
		...
		{
			if (current >= 0)
			FCEU_DispMessage("Recording frame %u",framecount);
			else
			FCEU_DispMessage("Playing frame %u / %i",framecount,MovieLoadedTotalFrames);
		}
...
// if(current > 0 || (!movie_readonly && current < 0))            /* Recording or Playback (!read-only) */
 if(current!=0 && !movie_readonly)
 {
  ...
  if(current < 0) {                   // switch to recording
   if (!LoadMovieStateError)					// only if the save state is from a movie
   	current = -current;
   else
   {
    LoadMovieStateError = 0;
   	return 0;
   }
  }
...
static int FCEUMOV_CheckWrongMovie(FILE* st, uint32 size)
{
	//movies with size=0 make this function crash
 if (!size || !moviedatasize || size < 5 || moviedatasize < 5) return;

 static int randTester;
 static int counter;
 static int testerSize;
 static int testerOffset;
 static uint8* moviedatastate = NULL;
 moviedatastate = (uint8*)realloc(moviedatastate, size);
 static int newmoviedatasize;
 newmoviedatasize = moviedatasize;

	//read movie within savestate
 fpos_t pos;
 fgetpos(st,&pos);
 if(size && fread(moviedatastate, 1, size, st)<size)
  return 0;
 fsetpos(st,&pos);

	//ignore the last four bytes
 size-=5;
 newmoviedatasize-=5;

	//if the savestate size is bigger, then the offset is X
 if (size > newmoviedatasize) testerOffset = size-newmoviedatasize;
 else testerOffset = newmoviedatasize-size;
	//if the savestate size is bigger, then the memory range to be compared will be X size
 if (size > newmoviedatasize) testerSize = size-testerOffset;
 else testerSize = newmoviedatasize-testerOffset;

 srand(time(NULL));
 for(counter=0; counter<50; counter++)
 {
 	randTester = rand() % testerSize + testerOffset;
  if (moviedata[testerSize+testerOffset-randTester] != moviedatastate[testerSize+testerOffset-randTester])
 	{
 	 WrongMovie = 1;
 	 break;
 	}
...
void FCEUMOV_ResetStopframe(int opt)
{
 if (opt) stopframe = MovieLoadedTotalFrames;
 else stopframe = 0;
}

...
static int SubLoad(FCEUFILE *fp)
{
 ...
 if (!DisableMD5Update) md5_starts(&md5);
 ...
 for(x=0;x<TotalSides;x++)
 {
  ...
  if (!DisableMD5Update) md5_update(&md5,diskdata[x],65500);
 }
 if (!DisableMD5Update) md5_finish(&md5,FCEUGameInfo->MD5);
 else DisableMD5Update = 0;
 return(1);
}

...
static char *DOvers[7]={0,0,0,0,0,0,0};
...
void SetDirs(void)
{
 int x;
 static int jlist[7]=
  {FCEUIOD_CHEATS,FCEUIOD_MISC,FCEUIOD_NV,FCEUIOD_STATE,FCEUIOD_SNAPS, FCEUMKF_FDSROM, FCEUIOD__COUNT};
 ...
 if(DOvers[6])
  FCEUI_SetBaseDirectory(DOvers[6]);
 else
...
void RemoveDirs(void)
{
 ...
 for(x=0;x<5;x++)
  if(!DOvers[x])
  {
   sprintf(TempArray,"%s\\%s",DOvers[6]?DOvers[6]:BaseDirectory,defaultds[x]);

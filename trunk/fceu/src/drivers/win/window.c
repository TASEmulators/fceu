...
static int MemToolsUpdateSpeed=100;
static int CheckedMemToolsUpdateSpeed=40031;
static int EnablePauseAfterPlayback=0;
static int EnableStopMessageInAvi=0;
...
void UpdateMenu(void)
{
	...
	CheckMenuItem(fceumenu,40034, EnablePauseAfterPlayback?MF_CHECKED:MF_UNCHECKED);
	CheckMenuItem(fceumenu,40035, EnableStopMessageInAvi?MF_CHECKED:MF_UNCHECKED);
	...
	int MemToolsUpdateSpeedIDs[]={40031,40032,40033,0};
	int MemToolsUpdateSpeedOptions[]={100,25,999,0};
	x = 0;
	while(MemToolsUpdateSpeedIDs[x])
	{
		CheckMenuItem(fceumenu,MemToolsUpdateSpeedIDs[x],
			MemToolsUpdateSpeedOptions[x]==MemToolsUpdateSpeed?MF_CHECKED:MF_UNCHECKED);
		x++;
	}
}
    ...
    case WM_COMMAND:
                if(!(wParam>>16))
                {
				 ...
				 case 40024:
					 ...
					 break;
				 case 40031:
				 	 MemToolsUpdateSpeed = 100;
					 CheckedMemToolsUpdateSpeed = wParam;
					 UpdateMenu();
					 break;
				 case 40032:
					 MemToolsUpdateSpeed = 25;
					 CheckedMemToolsUpdateSpeed = wParam;
					 UpdateMenu();
					 break;
				 case 40033:
					 MemToolsUpdateSpeed = 999;
					 CheckedMemToolsUpdateSpeed = wParam;
					 UpdateMenu();
					 break;
				 case 40016:
				 ...
				 case 40025:
				  ...
				  case 40029:
					  ...
					  break;
				  case 40035:
					  EnableStopMessageInAvi^= 1;
					  FCEUAVI_ShowMovieAVIMessage(EnableStopMessageInAvi);
					  UpdateMenu();
					  break;
				  case 40034:
					  EnablePauseAfterPlayback^= 1;
					  FCEUMOV_ResetStopframe(EnablePauseAfterPlayback);
					  UpdateMenu();
					  break;
...
static BOOL CALLBACK DirConCallB(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch(uMsg){
   ...
   case WM_COMMAND:
                if(!(wParam>>16))
                {
                 if((wParam&0xFFFF)>=200 && (wParam&0xFFFF)<=208)
                 {
                  static char *helpert[9]={"Cheats","Miscellaneous","Nonvolatile Game Data","Save States","Screen Snapshots","FDS BIOS ROM Image","Memory Watch","Basic Bot","Base Directory"};
                  char name[MAX_PATH];

...
static BOOL CALLBACK DirConCallB(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  switch(uMsg){
   ...
   case WM_COMMAND:
                if(!(wParam>>16))
                {
                 if((wParam&0xFFFF)>=200 && (wParam&0xFFFF)<=206)
                 {
                  static char *helpert[7]={"Cheats","Miscellaneous","Nonvolatile Game Data","Save States","Screen Snapshots","FDS BIOS ROM Image","Base Directory"};
                  char name[MAX_PATH];
